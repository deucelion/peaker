...
import { OverlayFooter, OverlaySheet, OVERLAY_Z } from "@/components/ui/overlay";
...
export function SyncStatusCenter({
  ...
  return (
    <OverlaySheet
      open={open}
      onClose={onClose}
      layer={OVERLAY_Z.DRAWER_PRIORITY}
      titleId="sync-status-center-title"
      shellClassName="ui-card flex max-h-[min(90dvh,680px)] w-full max-w-lg flex-col rounded-t-2xl shadow-2xl sm:rounded-2xl !max-w-lg !p-0"
    >
      <div className="border-b border-white/5 px-4 py-3">
        <div className="flex items-center justify-between gap-2">
          <h2 id="sync-status-center-title" className="text-sm font-black uppercase tracking-wide text-white">
            Senkron durumu
          </h2>
        ...
        <div className="mt-2 flex flex-wrap gap-2 text-[9px] font-black uppercase">
          <span className="ui-kpi-band rounded-md px-2 py-0.5 text-gray-400">Bekleyen {stats.total}</span>
          {stats.failed > 0 ? (
        ...
        {items.length === 0 ? (
          ...
          <ul className="space-y-2">
            {items.map((item) => {
              ...
              return (
                <li key={item.id} className="ui-card-inner px-3 py-2.5">
                  ...
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {needsUserConfirm && onConfirmOne ? (
                      <button
                        ...
                        disabled={syncing}
                        className="ui-btn-primary min-h-9 px-2.5 text-[9px] disabled:opacity-50"
                      >
                        Onayla ve gönder
                      </button>
                    ) : null}
                    {canRetry ? (
                      <button
                        ...
                        disabled={syncing}
                        className="ui-btn-ghost min-h-9 border-[color:color-mix(in_srgb,var(--peaker-ui-PRIMARY)_30%,transparent)] px-2.5 text-[9px] text-[color:color-mix(in_srgb,var(--peaker-ui-PRIMARY)_70%,white)] disabled:opacity-50"
                      >
                        Tekrar dene
                      </button>
                    ) : null}
                    {item.navigationHref ? (
                      <Link
                        ...
                        onClick={onClose}
                        className="ui-btn-ghost inline-flex min-h-9 items-center gap-1 px-2.5 text-[9px] text-gray-300"
                      >
                    ...
                    <button
                      ...
                      className="ui-btn-ghost ml-auto inline-flex min-h-9 items-center gap-1 px-2.5 text-[9px] text-gray-500"
                    >
      ...
      <OverlayFooter>
        <button
          ...
          onClick={onRetry}
          className="ui-btn-primary inline-flex min-h-11 flex-1 items-center justify-center gap-2 disabled:opacity-50"
        >
          {syncing ? (
