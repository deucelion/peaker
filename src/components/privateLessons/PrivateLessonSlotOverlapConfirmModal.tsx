...
export function PrivateLessonSlotOverlapConfirmModal({
  ...
  return (
    <OverlayDialog
      ...
      strongBackdrop
      titleId="slot-overlap-title"
      shellClassName="flex max-h-[min(92dvh,640px)] w-full max-w-md flex-col overflow-hidden rounded-2xl  shadow-[0_24px_60px_-28px_rgba(0,0,0,0.95)] !max-w-md !p-0"
    >
        <div className="flex-1 overflow-y-auto p-5 sm:p-6">
          <p className="text-[10px] font-black uppercase tracking-wider ui-kpi-card__trend">Koç slot uyarısı</p>
          ...
          <ul className="mt-4 space-y-1.5 rounded-xl ui-card-inner p-4 text-sm font-bold text-gray-200">
          ...
          {capacityMsg ? (
            <p
              className={`mt-4 rounded-xl border px-4 py-3 text-[11px] font-bold leading-relaxed ${ isCritical ? "border-red-500/40 bg-red-500/10 text-red-100" : isWarning ? "border-amber-500/35 bg-amber-500/10 text-amber-100" : "bg-white/5 text-gray-300" }`}
            >
              {capacityMsg}
            </p>
          ) : null}
