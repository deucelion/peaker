...
export async function buildMeRolePayload(): Promise<MeRoleSuccess | MeRoleFailure> {
  try {
    ...
    // FAZ 29: super_admin yalnızca profil rolünden türetilir; metadata claim'i
    // super_admin'e yükseltme yapamaz.
    const effectiveProfile = looksLikeSuperAdminRole(profile?.role)
      ? {
