...
export function applyFootersToAllPages(doc: jsPDF, ctx: PdfTextContext): void {
  ...
  for (let i = 1; i <= total; i += 1) {
    ...
    pdfSetFont(doc, ctx, "normal");
    doc.text(pdfT("Peaker Performans Raporu — Kurumsal", ctx), MARGIN, pageH - 5);
...
export function drawEmbeddedChartImage(
  ...
  try {
    ...
    doc.setFontSize(8);
    doc.text(pdfT("Grafik gomulemedi.", ctx), x, y + 4);
  }
  return y + h + 6;
}
