...
import { ChartNoData, chartTooltipStyle } from "@/components/ui/charts";
...
export default function GenelTakimRaporu() {
  ...
  if (loading && stats.length === 0 && !loadError) return (
    <div className="min-h-[50dvh] px-4 flex flex-col items-center justify-center bg-black gap-6 min-w-0 overflow-x-hidden pb-[max(env(safe-area-inset-bottom,0px),0.5rem)] text-center">
      <Loader2 className="w-12 h-12 text-[color:var(--peaker-ui-PRIMARY)] animate-spin" aria-hidden />
  ...
  return (
    <div className="space-y-6 sm:space-y-8 text-white pb-[max(5rem,env(safe-area-inset-bottom,0px))] min-w-0 overflow-x-hidden">
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:justify-between lg:items-center min-w-0">
        <Link href="/saha-testleri" className="flex items-center gap-3 sm:gap-4 text-gray-500 sm:hover:text-[color:var(--peaker-ui-PRIMARY)] transition-all group self-start min-h-11 touch-manipulation">
          <div className="p-3 ui-card rounded-2xl sm:group-hover:border-[color:color-mix(in_srgb,var(--peaker-ui-PRIMARY)_50%,transparent)] transition-all shadow-xl shrink-0">
        ...
        <div className="text-center min-w-0 flex-1 order-first lg:order-none">
          <h1 className="text-3xl sm:text-4xl font-black italic uppercase tracking-tighter leading-tight break-words">
            TAKIM <span className="text-[color:var(--peaker-ui-PRIMARY)]">ANALİZİ</span>
          </h1>
          <div className="flex items-center justify-center gap-2 sm:gap-3 mt-3 sm:mt-4 flex-wrap px-2">
             <div className="h-[1px] w-6 sm:w-8 ui-btn-primary/30 shrink-0" />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 min-w-0">
        ...
        <SummaryCard title="Toplam Veri Girişi" val={stats.length.toString()} icon={<Activity size={20} />} color="text-[color:var(--peaker-ui-PRIMARY)]" />
        <SummaryCard title="Aktif Test Sayısı" val={availableTests.length.toString()} icon={<BarChart3 size={20} />} color="text-emerald-500" />
      </div>

      {/* TEST SEÇİCİ */}
      <div className="flex flex-wrap gap-2 p-2 ui-card rounded-xl w-full min-w-0">
        {availableTests.map((testName) => (
          <button
            type="button"
            key={testName}
            onClick={() => setSelectedTest(testName)}
            className={`min-h-10 flex-1 sm:flex-none px-4 py-2.5 rounded-lg font-black text-[10px] uppercase transition-all flex items-center justify-center gap-2 touch-manipulation min-w-0 ${ selectedTest === testName ? "ui-btn-primary text-white shadow-2xl shadow-[color:color-mix(in_srgb,var(--peaker-ui-PRIMARY)_20%,transparent)]" : "ui-tabs-nav__tab--inactive text-gray-500 sm:hover:text-white" }`}
          >
      ...
      {/* GRAFİK PANELİ */}
      <div className="ui-card rounded-2xl p-4 sm:p-5 md:p-6 shadow-xl relative overflow-hidden group min-w-0">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end mb-5 sm:mb-6 relative z-10 gap-4 sm:gap-5 min-w-0">
          <div className="space-y-3 sm:space-y-4 min-w-0 flex-1">
            <div className="flex items-center gap-3 min-w-0">
              <div className="w-10 h-10 shrink-0 ui-kpi-chip--brand rounded-xl flex items-center justify-center">
            ...
            <h4 className="text-xl sm:text-2xl md:text-3xl font-black italic uppercase text-white tracking-tighter break-words">
              ...
              <span className="text-[color:var(--peaker-ui-PRIMARY)] opacity-30 ml-2 sm:ml-4 text-base sm:text-xl block sm:inline mt-1 sm:mt-0">[{currentUnit}]</span>
            </h4>
          </div>
          
          <div className="p-4 sm:p-5 rounded-xl backdrop-blur-xl w-full sm:min-w-[220px] sm:w-auto sm:group-hover:border-[color:color-mix(in_srgb,var(--peaker-ui-PRIMARY)_30%,transparent)] transition-all shrink-0 min-w-0">
        ...
        <div className="ui-chart-shell h-[min(50vh,340px)] sm:h-[min(50vh,380px)] w-full min-h-[230px] relative z-10">
          {currentTestData.length > 0 ? (
                ...
                <Bar dataKey="deger" radius={[15, 15, 0, 0]} barSize={45}>
                  {currentTestData.map((entry, index) => (
                    <Cell 
                      key={`cell-${index}`} 
                      fill={
                        entry.deger === bestValue
                          ? "var(--peaker-ui-PRIMARY)"
                          : "color-mix(in srgb, var(--peaker-ui-PRIMARY) 12.5%, transparent)"
                      }
            ...
            <div className="flex h-full flex-col items-center justify-center p-6 text-center">
              <ChartNoData label="Bu test için henüz veri yok" />
              <p className="mt-1 text-[11px] font-bold text-gray-600">İlk veri girişini saha testleri ekranından yapabilirsiniz.</p>
              <Link href="/saha-testleri" className="ui-btn-ghost mt-3 inline-flex min-h-10 items-center justify-center rounded-lg px-3 text-[10px] font-black uppercase tracking-wide transition sm:hover:border-[color:color-mix(in_srgb,var(--peaker-ui-PRIMARY)_35%,transparent)] sm:hover:ui-kpi-card__trend">
                Veri girişine git
              </Link>
            </div>
          )}
        </div>
        <div className="absolute -bottom-24 -left-24 w-[500px] h-[500px] ui-kpi-band blur-[120px] rounded-full -z-0"></div>
      </div>
    </div>
  );
}
...
function SummaryCard({ title, val, icon, color }: { title: string; val: string; icon: ReactNode; color: string }) {
  return (
    <div className="ui-card p-4 sm:p-5 rounded-xl flex items-center justify-between gap-3 group sm:hover:border-[color:color-mix(in_srgb,var(--peaker-ui-PRIMARY)_20%,transparent)] transition-all shadow-xl relative overflow-hidden min-w-0">
      ...
      <div className={`ui-card-inner p-3 sm:p-4 shrink-0 rounded-lg ${color} sm:group-hover:scale-110 sm:group-hover:ui-kpi-chip--brand transition-all relative z-10`} aria-hidden>
        {icon}
      </div>
    </div>
  );
