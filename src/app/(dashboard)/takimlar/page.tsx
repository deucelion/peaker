    ...
    <div className="ui-page min-w-0 overflow-x-hidden pb-[max(4rem,env(safe-area-inset-bottom,0px))]">
      <header className="flex min-w-0 flex-col gap-4 border-b border-white/5 pb-6 md:flex-row md:items-end md:justify-between">
        <div className="min-w-0">
          <h1 className="ui-h1">
            TAKIM <span className="text-[color:var(--peaker-ui-PRIMARY)]">YONETIMI</span>
          </h1>
      ...
      <form onSubmit={onCreateTeam} className="mt-6 flex min-w-0 flex-col gap-3 rounded-[1.5rem] ui-card p-4 sm:flex-row sm:items-center sm:p-5">
        <input
          value={newTeamName}
          onChange={(e) => setNewTeamName(e.target.value)}
          placeholder="Yeni takim adi"
          maxLength={60}
          className="ui-input min-h-11 flex-1 text-base uppercase sm:text-xs"
          required
        />
        <button
          type="submit"
          disabled={submitting}
          className="inline-flex min-h-11 touch-manipulation items-center justify-center gap-2 rounded-xl ui-btn-primary px-5 py-3 text-[10px] font-black uppercase text-white disabled:opacity-60 sm:hover:opacity-90"
        >
      ...
      {loading ? (
        ...
        <div className="mt-8 rounded-[2rem] ui-card p-8 text-center">
        ...
        <div className="mt-6 grid min-w-0 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((team) => (
            <div
              key={team.id}
              className="group relative min-w-0 rounded-[1.5rem] ui-card p-5 transition-all sm:hover:border-[color:color-mix(in_srgb,var(--peaker-ui-PRIMARY)_30%,transparent)]"
            >
              ...
              <Link
                ...
                className="absolute bottom-4 right-4 inline-flex min-h-10 items-center gap-2 rounded-lg border ui-kpi-chip--brand px-3 py-2 text-[9px] font-black uppercase ui-kpi-card__trend opacity-100 transition-all md:opacity-0 md:group-hover:opacity-100"
              >
