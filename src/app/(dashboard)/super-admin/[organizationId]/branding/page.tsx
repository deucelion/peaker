...
import { loadOrganizationBrandingEditorSnapshot } from "@/lib/actions/organizationBrandingActions";
...
import OrgBrandingEditor from "../../_components/OrgBrandingEditor";

interface PageProps {
  params: Promise<{ organizationId: string | string[] | undefined }>;
}

export default async function SuperAdminOrganizationBrandingPage({ params }: PageProps) {
  ...
  const loaded = await loadOrganizationBrandingEditorSnapshot(organizationId);
  if (!loaded.ok) {
  ...
  return (
    <div className="space-y-5 sm:space-y-6 pb-[max(3rem,env(safe-area-inset-bottom,0px))] min-w-0 overflow-x-hidden">
      ...
      <OrgBrandingEditor initialSnapshot={loaded.snapshot} />
    </div>
  );
}
